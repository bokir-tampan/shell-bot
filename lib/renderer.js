/**
* Этот класс сохраняет логическое отображение строк в сообщениях.
 * На самом деле он не отображает и не отправляет сообщения, он делегирует
 * эта задача для рендерера.
 *
 * FIXME: сделайте что-нибудь, чтобы очень длинные сообщения
 * отправлено (и отклонено), если между ними вставлено слишком много строк
 * сообщение.
 **/
...
Renderer.prototype._linesRemoving = function _linesRemoving(y, n) {
  ...
  // Ищите, пока мы не дойдем до линии розыска
  y += this.offset;
  var idx = 0, lineIdx = 0;
  while (y) {
...
Renderer.prototype._linesScrolling = function _linesScrolling(n) {
  ...
  if (n > 0) {
    // Прокрутка вверх: увеличить смещение, при необходимости отменить сообщение
    this.offset += n;
    this.totalLines -= n;
    ...
    // Прокрутка вниз: просто удалите нижние строки (их оставление все усложнит)
    n = -n;
...
// Ищем, пока не дойдем до нужной строки, * непосредственно перед следующей *
  y += this.offset;
  var idx = 0, lineIdx = 0;
  while (true) {
    var lines = (idx === this.messages.length) ? this.orphanLines : this.messages[idx].lines;
...
Renderer.prototype.update = function update() {
 ...
 // Рендеринг сообщений, планирование сброса, если некоторые из них изменились
  ...
  // Убедитесь, что сиротские строки обрабатываются
...
/* Для данной строки вернуть истину, если она потенциально моноширинная */
Renderer.prototype.evaluateCode = function evaluateCode(str) {
// FIXME: строка между двумя строками кода должна быть кодом
  if (str.indexOf("   ") !== -1 || /[-_,:;<>()/\\~|'"=^]{4}/.exec(str))
    return true;
  return false;
};

/* Для данного объекта сообщения выполняется рендеринг в фрагмент HTML */
Renderer.prototype.render = function render(message) {
  ...
  var html = message.lines.map(function (line, idx) {
    ...
    if (idx === 0 && !content.substring(0, this.options.startFill.length).trim()) {
      // Сообщение будет начинаться с пробелов, которые будут обрезаны телеграммой
...
// Устанавливается при изменении существующей строки, отменяется, когда редактируемые строки сбрасываются
  this.editedLineTimer = new utils.Timer(this.options.editTime).on("fire", this.flushEdited.bind(this));

// Устанавливается при добавлении или изменении новой строки, отменяется при сбросе новых строк
  this.newChunkTimer = new utils.Timer(this.options.chunkTime).on("fire", this.flushNew.bind(this));
// Сброс при добавлении или изменении новой строки, отменяется при сбросе новых строк
  this.newLineTimer = new utils.Timer(this.options.lineTime).on("fire", this.flushNew.bind(this));

// Устанавливается, когда есть незаконченная непустая строка, отменяется, когда ссылка изменяется или строка становится пустой
